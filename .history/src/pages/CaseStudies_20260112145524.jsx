...
const caseStudies = [
  ...
  },
  {
    ...
    title: "Patient Management Portal for GI Disorders",
    description:
      "A comprehensive portal for tracking symptoms, treatments, and analytics for patients with chronic GI conditions.",
    problem:
      "Fragmented patient records and lack of personalized tracking led to poor adherence and delayed interventions.",
    approach:
      "User-centric design with secure data modeling and integration with wearable health devices.",
    solution:
      "Built a secure Next.js application with symptom trackers and predictive insights.",
    result:
      "Patient adherence increased by 68% and doctor visit efficiency improved by 55%.",
    impact:
      "Empowered patients with self-management tools and reduced hospital readmissions.",
    tech: [
      "Next.js",
      "Express.js",
      "JWT Auth",
  ...
  },
  {
    ...
    title: "Predictive Analytics Engine for GI Cancer Screening",
    description:
      "A data-driven system predicting GI cancer risks using genetic and clinical data.",
    problem:
      "Late-stage diagnoses due to insufficient risk assessment tools.",
    approach:
      "Aggregated multi-source data and applied ML algorithms.",
    solution:
      "Built dashboards with automated alerts and EHR integration.",
    result:
      "Risk prediction accuracy reached 89% and early screenings increased by 60%.",
    impact:
      "Revolutionized preventive gastroenterology care.",
    tech: [
      "Vue.js",
      "Python",
      "Flask",
  ...
  return (
    ...
    >
      <motion.div
        ...
        onClick={(e) => e.stopPropagation()}
        className="bg-white w-full max-w-6xl max-h-[92vh] rounded-3xl overflow-hidden shadow-2xl flex flex-col"
      >
        {/* Header */}
        <div className={`bg-gradient-to-br ${study.color} p-8 sm:p-12 text-white`}>
          <div className="flex flex-col sm:flex-row gap-6">
            {study.icon}
            <div>
              <h2 className="text-2xl sm:text-4xl font-black">{study.title}</h2>
              <p className="mt-3 opacity-90">{study.description}</p>
            </div>
          </div>
        </div>

        {/* Content */}
        <div className="flex-1 overflow-y-auto bg-gray-50 p-6 sm:p-10 space-y-8">
          {["Problem", "Approach", "Solution", "Result", "Impact"].map(
            (section) => (
              <motion.div key={section} variants={contentVariants}>
                <h3 className="text-lg sm:text-xl font-bold mb-2 border-b pb-1">
                  {section}
                </h3>
          ...
          <motion.div variants={contentVariants}>
            <h3 className="text-lg sm:text-xl font-bold mb-3 border-b pb-1">
              Technologies
            </h3>
            <div className="flex flex-wrap gap-2">
              {study.tech.map((t) => (
                <span
                  key={t}
                  className="px-3 py-1.5 bg-gray-200 rounded-full text-xs sm:text-sm"
                >
                  {t}
                </span>
              ))}
        ...
        <div className="p-5 border-t flex justify-end">
          <button
            onClick={onClose}
            className="px-6 py-2 rounded-xl bg-gray-900 text-white hover:bg-black active:scale-95 transition"
          >
            Close
          </button>
        </div>
    ...
    <LayoutGroup>
      <section className="py-16 sm:py-24 bg-gray-100 min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          {/* Hero */}
          <div className="text-center mb-16">
            <div className="flex justify-center gap-2 text-blue-600 mb-4">
              <FaTools />
            ...
            <p className="text-gray-500 max-w-3xl mx-auto">
              Transformative gastroenterology solutions powered by technology.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {caseStudies.map((study) => (
              <motion.div
                ...
                onClick={() => setActiveStudy(study)}
                className="bg-white rounded-3xl shadow-lg cursor-pointer border hover:shadow-xl transition"
              >
                <div className={`bg-gradient-to-br ${study.color} p-8 text-white`}>
                  {study.icon}
                  <h3 className="text-2xl font-bold mt-4">
