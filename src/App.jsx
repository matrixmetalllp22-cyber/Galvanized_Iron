...
function AppContent() {
  useEffect(() => {
    ...
    const disableShortcuts = (e) => {
      if (
        (e.ctrlKey &&
          ["u", "s", "c", "x", "i", "j"].includes(e.key.toLowerCase())) ||
        e.key === "F12"
      ) {
        e.preventDefault();
        alert("Action disabled for content protection.");
  ...
  return (
    <BrowserRouter>
      <div className="flex flex-col min-h-screen select-none">
        ...
        {/* ❌ pt-16 REMOVED — THIS FIXES THE TOP GAP */}
        <main className="grow bg-linear-to-br from-slate-50 to-white-300 transition-colors duration-300">
          <Routes>
            <Route path="/" element={<Home />} />
            <Route path="/about" element={<About />} />
            <Route path="/products" element={<Products />} />
